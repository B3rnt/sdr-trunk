...
public class DMRDecoderState extends TimeslotDecoderState
{
    ...
    private CorrectedBinaryMessage mCapacityMaxTalkerAliasHeaderFragment;
    ...
    private void processLinkControl(LCMessage message, boolean isTerminator)
    {
        switch(message.getOpcode())
        {
            ...
            case FULL_CAPACITY_MAX_TALKER_ALIAS:
                ...
                {
                    //Capture the raw payload fragment and (re)assemble with any continuation fragment we have.
                    //Note: the fragments must be combined into a single raw byte sequence and decoded once -- decoding
                    //each fragment independently can split a multi-byte (e.g. UTF-8) character across the fragment
                    //boundary and corrupt the decoded text (e.g. truncated Cyrillic aliases).
                    mCapacityMaxTalkerAliasHeaderFragment = alias.getAliasFragment();
            ...
            case FULL_CAPACITY_MAX_TALKER_ALIAS_CONTINUATION:
                ...
                {
                    mCapacityMaxTalkerAliasBlock1Fragment = alias.getAliasFragment();
                    updateCapacityMaxTalkerAlias();
                ...
                break;
            case FULL_CAPACITY_MAX_TALKER_ALIAS_BLOCK_2:
                if(message instanceof CapacityMaxTalkerAliasContinuation alias)
                {
                    mCapacityMaxTalkerAliasBlock2Fragment = alias.getAliasFragment();
                    updateCapacityMaxTalkerAlias();
                ...
                break;
            case FULL_CAPACITY_MAX_TALKER_ALIAS_BLOCK_3:
                if(message instanceof CapacityMaxTalkerAliasContinuation alias)
                {
                    mCapacityMaxTalkerAliasBlock3Fragment = alias.getAliasFragment();
                    updateCapacityMaxTalkerAlias();
    ...
    private void closeCurrentCallEvent(long timestamp)
    {
        ...
        mCapacityMaxTalkerAliasHeaderFragment = null;
    ...
    {
        if(mCapacityMaxTalkerAliasHeaderFragment == null)
        {
            return;
        }

        int totalBits = mCapacityMaxTalkerAliasHeaderFragment.size();

        if(mCapacityMaxTalkerAliasBlock1Fragment != null)
        {
            totalBits += mCapacityMaxTalkerAliasBlock1Fragment.size();
        }
        if(mCapacityMaxTalkerAliasBlock2Fragment != null)
        {
            totalBits += mCapacityMaxTalkerAliasBlock2Fragment.size();
        }
        if(mCapacityMaxTalkerAliasBlock3Fragment != null)
        {
            totalBits += mCapacityMaxTalkerAliasBlock3Fragment.size();
        }

        CorrectedBinaryMessage combined;

        if(totalBits == mCapacityMaxTalkerAliasHeaderFragment.size())
        {
            combined = mCapacityMaxTalkerAliasHeaderFragment;
        }
        else
        {
            combined = new CorrectedBinaryMessage(totalBits);
            int offset = 0;
            combined.load(offset, mCapacityMaxTalkerAliasHeaderFragment);
            offset += mCapacityMaxTalkerAliasHeaderFragment.size();

            if(mCapacityMaxTalkerAliasBlock1Fragment != null)
            {
                combined.load(offset, mCapacityMaxTalkerAliasBlock1Fragment);
                offset += mCapacityMaxTalkerAliasBlock1Fragment.size();
            }
            if(mCapacityMaxTalkerAliasBlock2Fragment != null)
            {
                combined.load(offset, mCapacityMaxTalkerAliasBlock2Fragment);
                offset += mCapacityMaxTalkerAliasBlock2Fragment.size();
            }
            if(mCapacityMaxTalkerAliasBlock3Fragment != null)
            {
                combined.load(offset, mCapacityMaxTalkerAliasBlock3Fragment);
            }
        }
        ...
        DmrTalkerAliasIdentifier updated = DmrTalkerAliasIdentifier.create(value);
